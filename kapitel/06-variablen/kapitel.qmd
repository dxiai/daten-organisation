...
::: {#def-parameter} 
Die Eingaben einer Funktion heissen **Parameter**. Eine Funktion **akzeptiert** diese Parameter als Eingabe.
:::
...
::: {#def-argument}
Ein **Argument** heisst ein Wert, der einem Parameter zugewiesen wird.
:::
...
Formal lässt sich diese Definition wird der durch die Rückgabeliste definierte Zielbereich dem Bezeichner $E$ zugewiesen und den Definitionsbereich der Parameterliste mit $P$ gekennzeichnet, dann ist entsprechend dieser Definition **jede** Funktion entsprechend @eq-fkt-def definiert. 
...
### Precedence - Priorität von Operatoren
...
In der Mathematik werden Indices durch tieferstellen gekennzeichnet.  Ein Index wird dann verwendet, wenn ein Bezeichner auf eine Datenstruktur verweist. Der Index bezeichnet den Wert an der entsprechenden Position in der Datenstruktur. Am Beispiel des Summe-Operators wird dies in @eq-index-sum verdeutlicht.
...
Die erzeugten Funktionen lassen sich wie andere Funktionen über ihre Bezeichner anwenden. Im aktuellen Beispiel ergeben sich so die Gleichungen in @eq-closure-results.
...
Diese Beziehung erfüllt die Kriterien für das neutrale Element aus @eq-neutrales-element für den allgemeinen Verkettungsoperator.
